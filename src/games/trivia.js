...
export function getTriviaSettings(userId) {
  const saved = configOps.get(`trivia_settings_${userId}`);
  return saved ? JSON.parse(saved) : { ...DEFAULT_SETTINGS };
}

// Save user's trivia settings
export function saveTriviaSettings(userId, settings) {
  configOps.set(`trivia_settings_${userId}`, JSON.stringify(settings));
}

// Get user's trivia stats
export function getTriviaStats(userId) {
...
export function updateTriviaStats(userId, correct, points = 0) {
  ...
  configOps.set(`trivia_stats_${userId}`, JSON.stringify(stats));
  return stats;
}
...
async function endGame(channel, channelId) {
  ...
  for (const odId of session.scores.keys()) {
    const stats = getTriviaStats(odId);
    stats.gamesPlayed++;
    configOps.set(`trivia_stats_${odId}`, JSON.stringify(stats));
  }
